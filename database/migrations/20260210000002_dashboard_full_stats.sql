  ...
  SELECT json_build_object(
    ...
    'tasks', (
      ...
      FROM (
        ...
        ORDER BY
          CASE
            WHEN due_date < v_now THEN 1  -- 已过期
            WHEN due_date <= v_now + INTERVAL '3 days' THEN 2  -- 3天内
            WHEN due_date <= v_now + INTERVAL '7 days' THEN 3  -- 一周内
            ELSE 4  -- 更远
          END,
          due_date ASC
      ) t
    ...
    'stats', (
      SELECT json_build_object(
        ...
        'totalSuppliers', (SELECT count(*)::int FROM public.suppliers),
        'activeSuppliers', (SELECT count(*)::int FROM public.suppliers WHERE status = 'active'),
        'averageScore', (
          SELECT COALESCE(ROUND(AVG(system_score), 1), 0)::numeric(10,2)
          FROM public.suppliers
          WHERE system_score IS NOT NULL
        )
      )
    ),
    ...
    'businessStats', (
      SELECT json_build_object(
        ...
        'totalSuppliers', (SELECT COUNT(*)::int FROM public.suppliers),
        'pendingTasks', (SELECT COUNT(*)::int FROM public.tasks WHERE status = 'pending'),
        'activeSuppliers', (SELECT COUNT(*)::int FROM public.suppliers WHERE status = 'active'),
        'strategicSuppliers', (SELECT COUNT(*)::int FROM public.suppliers WHERE tier = 'strategic'),
        'coreSuppliers', (SELECT COUNT(*)::int FROM public.suppliers WHERE tier = 'core')
      )
    ),
